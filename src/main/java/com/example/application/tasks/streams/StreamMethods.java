...
public class StreamMethods {
    public double getAverageValue(List<Integer> list) {
        return list.stream().mapToInt(Integer::intValue).average().orElse(0.0);
    }

    public List<String> transformListOfStrings(List<String> list) {
        return list.stream().map(str -> "_new_" + str.toUpperCase()).collect(Collectors.toList());
    }

    public List<Integer> getListOfSquares(List<Integer> list) {
    ...
    public <T> T getLastElement(Collection<T> collection) throws EmptyCollectionException {
        return collection.stream().reduce((i, j) -> j).orElseThrow(EmptyCollectionException::new);
    }

    public List<String> getStringsStartingWithLetter(Collection<String> collection, char letter) {
